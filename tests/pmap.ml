...
let is_page_protected (pm: (int, int) Store.t) pr (spm: (int, int) Store.t) ps pg envs pp = 
  check_pp pm pr spm ps pg envs pp; 
  Store.get pr pp != 0
  (*mem.(pp).protected != 0*)
...
let page_incref pm pr spm ps pg envs pp id =
  let xx = Store.get ps pp in (*cheating*)
  let yy = Store.get pr pp in (*cheating*)
  assert (not (is_page_protected pm pr spm ps pg envs pp));
  let pm = Store.set pm id pp in
  let pr' = Store.set pr pp id in (* was below next line but must be above *)
  let spm = Store.set spm id pp in
  let _ = show pp in
  let _ = show pr in
  let _ = show pr' in
  let _ = show pm in
...
let env_check pm pr spm ps (pg: (int*int, int) Store.t) envs env =
...
let env_alloc (ctr: int) (pm: (int, int) Store.t) (pr: (int, int) Store.t) (spm: (int, int) Store.t) (ps: (int, int) Store.t) (pg: (int * int, int) Store.t) (envs: (int, bool) Store.t) = 
  let env_pp = page_getfree pm pr spm ps pg envs in
  if env_pp = -1 then (pm, pr, spm, ps, pg, envs, Nothing) else
    let id = fresh_id ctr in
    let (pm, pr, spm, ps, pg, envs) = page_incref pm pr spm ps pg envs env_pp id in
    let spm = Store.set spm id env_pp in
    let pm = Store.set pm id env_pp in (*shadowed*)
