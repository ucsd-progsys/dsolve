...
let rec height = function
  | Empty -> 0
  | Red (e, l, r) -> if height (l) > height (r) then height (l) else height (r)
  | Black (e, l, r) -> (if height (l) > height (r) then height (l) else height (r)) + 1

let show x = x

let restore_right e lt r = match r with
  | Red (re, rl, rr) ->
...
let restore_left e l rt = match l with
  | Red (le, ll, lr) ->
