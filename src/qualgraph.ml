...
type graph_constraint =
    FlowsTo of
      constraint_vertex * flowlabel * constraint_vertex
...
let reshape_constraints c =
  let rec reshape_constraints_rec c w =
    ...
    match w with
      [] ->
	c
    (* pmr: degenerate cases; we should really do something about these. (The right
       thing to do is use type information to not be creating depend edges from function
...
let rec make_qualgraph = function
    FlowsTo(t1, f, t2)::w ->
	  ...
	  (ConstVertex v1, ConstVertex v2) ->
	    ...
	    let new_edge = FlowGraph.E.create v1 f v2 in
...
let expr_constraints exp bot =
  let rec expr_constraints_rec e qm =
	...
	| App(e1, e2, _) ->
	    ...
	    let retc = FlowsTo(v_out, (Positive, Call c), ve) in
