  ...
  and norm_out exp =
    ...
    match exp.pexp_desc with
     ...
     | Pexp_let(r, [(pat, e1)], e2) ->
        let ls = norm_in e1 in
        let (e_1, ls) = resolve_in_exp ls in
        let init = mk_let_gen r pat e_1 (norm_out e2) in
     ...
     | Pexp_record(es, None) ->
        ...
        (*let (es, se) = List.fold_right (fun (s, e) (ess, lss) -> let (e', ls) = resolve_in_exp (norm_in e) in
                                                                 ((s, e')::ess, ls::lss)) ee ([], []) in*)
