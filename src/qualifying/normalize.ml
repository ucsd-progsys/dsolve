...
let rec normalize_structure sstr =
  let _ = if !Clflags.verbose then Format.set_margin 170 in
  let _ = if !Clflags.verbose then Format.set_max_indent 150 in
  match sstr with
    [] -> []
    | {pstr_desc = (Pstr_eval exp); pstr_loc = loc} :: srem ->
        let normal_exp = normalize exp in
        let _ = if !Clflags.verbose then printf "@[%a@\n@]" Qdebug.pprint_expression normal_exp in
