...
let abs _x_ = if lt_int _x_ 0 then let _y_ = 0 - _x_ in _y_ else _x_ in
let queen _1_size =
begin
  ...
  let queenPrint _none =
    ...
    let rec _1_aux _1_row = 
		begin
      if eq_int _1_row _1_size then () else
  ...
  let test _2_j =
    ...
    let rec _2_aux _2_i =
		begin
      if lt_int _2_i _2_j then
        let q2i = Array.get queenArray _2_i in
				let qdiff = q2j - q2i in
				let absqdiff = abs qdiff in
        if q2i = q2j then false else if eq_int absqdiff _2_j - _2_i then false else 
  ...
  let rec loop _2_row =
    ...
    let next = _get_queenArray_2_row + 1 in
      if lt_int _1_size next then
        begin assign _2_row 0; if eq_int _2_row 0 then () else loop _2_row_minus end
      else
      begin 
				assign _2_row next;
        if test _2_row then
				begin
          if eq_int _2_row_plus _1_size then begin queenPrint (); loop _2_row end else loop _2_row_plus
				end
        else loop _2_row
      end  
	in loop 0
