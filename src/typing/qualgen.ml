...
let mk_intset il = 
  ...
  List.fold_left (fun i is -> CS.add i is) init il
    
let mk_idset pl =
  List.fold_left (fun (_, id) ids -> IS.add (Ident.name id) ids) IS.empty pl

let mk_tymap pl =
  let bind (t, id) tymap =
    let id = Ident.name id in
    TM.add t (IS.add id (findm t)) tymap in 
...
let rec visit_binding n (pat, expr) = 
  if is_function exp then visit_expr (n+1) expr
     else (lq n p; visit_expr n expr) 
...
and visit_expr n exp =
  ...
  let vb b = 
    ...
    let _ = C.addp ps in
     C.addl il is in 
  let bi p = addp (bound_idents n pat) in
  ...
  let rec ve expr =
    ...
    | Texp_apply (e, el) ->
        ve e; List.iter C.resl_opt el
    | Texp_match (e1, pel, _) ->
        ve e1; List.iter (fun (p, e) -> lq n p; ve e) pel 
    | Texp_array (el) ->
    ...
    | Texp_ifthenelse (e1, e2, e3) ->
        ve e1; ve e2; C.resl_opt ve e3
    | Texp_constant (_)
    | Texp_assertfalse
    | Texp_ident (_, _) ->
        () 
    | Texp_variant (_, _) -> assert false
    | Texp_try(_, (_, _)) -> assert false
    | _ -> assert false in
  ve exp; (!pl, !il) 
...
let visit_str sstr = 
  ...
  let visit_str_rec sstr 
    match sstr with
     Tstr_value (_, bl) :: srem ->
       List.fold_left vb (visit_str_rec srem) bl 
     | _ :: srem ->
       visit_str_rec srem
     | [] -> ([], []) in
  
  let (pl, il) = visit_str_rec sstr in  
